package com.enterprise.openfinance.fxservices.infrastructure.functional;
...
class FxUatTest {
    ...
    void shouldCompleteQuoteToDealJourney() {
        ...
        request()
                .header("X-Idempotency-Key", "IDEMP-UAT-FX-SERVICES-1")
                .contentType("application/json")
                .body("""
                        {
                          "Data": {
    ...
    void shouldRejectExpiredQuote() {
        request()
                .header("X-Idempotency-Key", "IDEMP-UAT-FX-SERVICES-EXP")
                .contentType("application/json")
                .body("""
                        {
                          "Data": {
    ...
    void shouldRejectUnsupportedAuthorizationHeader() {
        given().baseUri("http://localhost").port(port)
                ...
                .header("X-FAPI-Interaction-ID", "ix-fx-services-functional")
