package com.enterprise.openfinance.dynamiconboarding.infrastructure.functional;
...
class OnboardingUatTest {
    ...
    void shouldCompleteOnboardingJourneyWithReplay() {
        Response created = request()
                .header("X-Idempotency-Key", "IDEMP-UAT-DYNAMIC-ONBOARDING-1")
    ...
    void shouldRejectDecryptionFailureAndSanctionsHit() {
        request()
                .header("X-Idempotency-Key", "IDEMP-UAT-DYNAMIC-ONBOARDING-BAD")
                .contentType("application/json")
                .body("""
                        {
                          "Data": {
        ...
        request()
                .header("X-Idempotency-Key", "IDEMP-UAT-DYNAMIC-ONBOARDING-SANCTION")
                .contentType("application/json")
                .body("""
                        {
                          "Data": {
    ...
    void shouldRejectUnsupportedAuthorizationHeader() {
        given().baseUri("http://localhost").port(port)
                ...
                .header("X-FAPI-Interaction-ID", "ix-dynamic-onboarding-functional")
                .header("x-fapi-financial-id", "TPP-001")
                .header("X-Idempotency-Key", "IDEMP-UAT-DYNAMIC-ONBOARDING-INVALID")
    ...
    @ComponentScan(basePackages = {
            "com.enterprise.openfinance.dynamiconboarding.application",
            "com.enterprise.openfinance.dynamiconboarding.infrastructure"
    })
