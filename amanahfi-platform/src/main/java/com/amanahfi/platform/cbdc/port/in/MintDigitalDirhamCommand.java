...
/**
 * Command to mint new Digital Dirham (Central Bank only)
 */
@Value
@Builder
public class MintDigitalDirhamCommand implements Command {
    ...
    public void validate() {
        ...
        if (!amount.getCurrency().getCurrencyCode().equals("AED")) {
            throw new IllegalArgumentException("Digital Dirham minting must be in AED");
        }
        ...
        // Large minting requires additional validation
        if (amount.getAmount().compareTo(new BigDecimal("1000000000")) > 0) { // 1 billion AED
            if (monetaryPolicyReference == null || monetaryPolicyReference.trim().isEmpty()) {
                throw new IllegalArgumentException("Monetary policy reference required for large minting operations");
            }
        }
    }
}